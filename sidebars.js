/** @type {import('@docusaurus/plugin-content-docs').SidebarsConfig} */

const sidebars = {
  waf_sidebar: [
    {
      type: 'category',
      label: 'Web Application Firewall',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/waf'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/waf'
        }
      ]
    },
    {
      type: 'category',
      label: 'WAF Troubleshooting Guide',
      collapsed: false,
      link: {
        type: 'doc',
        id: 'troubleshooting'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'troubleshooting'
        }
      ]
    },
    {
      type: 'html',
      value: '<br><br>',
      defaultStyle: true,
      className: 'sidebar-title'
    },
    {
      type: 'html',
      value: '<br>Knowledge Base<br>',
      defaultStyle: true,
      className: 'sidebar-title', // Use the default menu item styling
    },
    {
      type: 'doc',
      id: 'kb/waf/waf'
    }
  ],
  adc_sidebar: [
    {
      type: 'category',
      label: 'Application Delivery Controller',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/adc'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/adc'
        }
      ]
    },
    {
      type: 'category',
      label: 'Deployment Scenarios',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'scenarios/adc'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'scenarios/adc'
        }
      ]
    },
    {
      type: 'html',
      value: '<br><br>',
      defaultStyle: true,
      className: 'sidebar-title'
    },
    {
      type: 'html',
      value: '<br>Knowledge Base<br>',
      defaultStyle: true,
      className: 'sidebar-title', // Use the default menu item styling
    },
    {
      type: 'doc',
      id: 'kb/adc/adc'
    }
  ],
  platform_sidebar: [
    {
      type: 'category',
      label: 'Haltdos Platform',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/platform'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/platform'
        }
      ]
    },
    {
      type: 'html',
      value: '<br><br>',
      defaultStyle: true,
      className: 'sidebar-title'
    },
    {
      type: 'html',
      value: '<br>Knowledge Base<br>',
      defaultStyle: true,
      className: 'sidebar-title', // Use the default menu item styling
    },
    {
      type: 'doc',
      id: 'kb/platform/platform'
    }
  ],
  llb_sidebar: [
    {
      type: 'category',
      collapsed: true,
      label: 'Link Load Balancer',
      link: {
        type: 'doc',
        id: 'enterprise/llb'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/llb'
        }
      ]
    },
    {
      type: 'category',
      label: 'Deployment Scenarios',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'scenarios/llb'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'scenarios/llb'
        }
      ]
    },
  ],
  scan_sidebar: [
    {
      type: 'category',
      label: 'Haltdos Vulnerability Scanner',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/scan'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/scan'
        }
      ]
    }
  ],
  plugins_sidebar: [
    {
      type: 'category',
      label: 'Haltdos Plugins',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/plugins'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/plugins'
        }
      ]
    }
  ],
  ddos_sidebar: [
    {
      type: 'category',
      label: 'Anti-DDOS',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/ddos'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/ddos'
        }
      ]
    }
  ],
  gslb_sidebar: [
    {
      type: 'category',
      label: 'Global Server Load Balancer',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/gslb'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/gslb'
        }
      ]
    },
    {
      type: 'html',
      value: '<br><br>',
      defaultStyle: true,
      className: 'sidebar-title'
    },
    {
      type: 'html',
      value: '<br>Knowledge Base<br>',
      defaultStyle: true,
      className: 'sidebar-title', // Use the default menu item styling
    },
    {
      type: 'doc',
      id: 'kb/gslb/gslb'
    }
  ],
  vpn_sidebar: [
    {
      type: 'category',
      label: 'Remote Access Gateway',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/vpn'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/vpn'
        },
      ]
    }
  ],
  ipam_sidebar: [
    {
      type: 'category',
      label: 'IPAM',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'enterprise/ipam'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'enterprise/ipam'
        }
      ]
    }
  ],
  kb_sidebar: [
    {
      type: 'category',
      label: 'Knowledge Base',
      link: {
        type: 'doc',
        id: 'kb'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'kb'
        }
      ]
    },
  ],
  mssp_sidebar: [
    {
      type: 'category',
      label: 'MSSP',
      link: {
        type: 'doc',
        id: 'mssp'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'mssp'
        }
      ]
    },
  ],
  cloud_sidebar: [
    {
      type: 'category',
      label: 'Haltdos Cloud',
      collapsed: true,
      link: {
        type: 'doc',
        id: 'cloud'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'cloud'
        }
      ]
    },
  ]
};

module.exports = sidebars;